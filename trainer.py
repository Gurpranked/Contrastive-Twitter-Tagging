...
from data_preprocess.data_preprocess_text import prep_text, get_averaged_embedding, obtain_transformation, convert_batch_to_embeddings
...
def setup_dataloaders(args):
    train_loader, val_loader, test_loader, n_classes = prep_text(args.batch_size, args.split_ratio)

    return [train_loader], val_loader, test_loader, n_classes


def setup_linclf(n_classes, DEVICE, bb_dim):
    '''
    @param bb_dim: output dimension of the backbone network
    @return: a linear classifier
    '''
    classifier = Classifier(bb_dim=bb_dim, n_classes=n_classes)
...
def setup_model_optm(args, n_classes, DEVICE, classifier=True):
    # set up backbone network
    if args.backbone == 'FCN':
        backbone = FCN(n_channels=args.n_feature, n_classes=n_classes, backbone=True)
    elif args.backbone == 'DCL':
        backbone = DeepConvLSTM(n_channels=args.n_feature, n_classes=n_classes, conv_kernels=64, kernel_size=5, LSTM_units=128, backbone=True)
    elif args.backbone == 'LSTM':
        backbone = LSTM(n_channels=args.n_feature, n_classes=n_classes, LSTM_units=128, backbone=True)
    elif args.backbone == 'AE':
        backbone = AE(n_channels=args.n_feature, len_sw=args.len_sw, n_classes=n_classes, outdim=128, backbone=True)
    elif args.backbone == 'CNN_AE':
        backbone = CNN_AE(n_channels=args.n_feature, n_classes=n_classes, out_channels=128, backbone=True)
    elif args.backbone == 'Transformer':
        backbone = Transformer(n_channels=args.n_feature, len_sw=args.len_sw, n_classes=n_classes, dim=128, depth=4, heads=4, mlp_dim=64, dropout=0.1, backbone=True)
    else:
        NotImplementedError
    ...
    if classifier:
        ...
        classifier = setup_linclf(n_classes, DEVICE, bb_dim)
...
def setup(args, n_classes, DEVICE):
    ...
    model, classifier, optimizers = setup_model_optm(args, n_classes, DEVICE, classifier=True)
    ...
    args.model_name = 'try_scheduler_' + args.framework + '_n_epoch' + str(args.n_epoch) + '_lr' + str(args.lr) + '_bs' + str(args.batch_size) \
    ...
    # TESTING CROSS ENTROPY LOSS FUNCTIONS FOR THE LINEAR CLASSIFIER
    criterion_cls = nn.CrossEntropyLoss() if True else custom_cross_entropy_loss()
...
def calculate_model_loss(args, sample_strings, targets, model, criterion, DEVICE, recon=None, nn_replacer=None):
    aug_samples1 = []
    aug_samples2 = []
    for data in sample_strings:
        aug_samples1.append(get_averaged_embedding(gen_aug(data, args.aug1), obtain_transformation()))
        aug_samples2.append(get_averaged_embedding(gen_aug(data, args.aug2), obtain_transformation()))
    aug_samples1, aug_samples2, targets = [aug_sample1.to(DEVICE).float() for aug_sample1 in aug_samples1], \
                                            [aug_sample2.to(DEVICE).float() for aug_sample2 in aug_samples2], \
                                                [target.to(DEVICE).long() for target in targets]
    stacked_aug_samples1 = torch.stack(aug_samples1)
    stacked_aug_samples2 = torch.stack(aug_samples2)
    ...
    if args.framework in ['byol', 'simsiam', 'nnclr']:
        if args.backbone in ['AE', 'CNN_AE']:
            x1_encoded, x2_encoded, p1, p2, z1, z2 = model(x1=stacked_aug_samples1, x2=stacked_aug_samples2)
            recon_loss = recon(stacked_aug_samples1, x1_encoded) + recon(stacked_aug_samples2, x2_encoded)
        else:
            p1, p2, z1, z2 = model(x1=stacked_aug_samples1, x2=stacked_aug_samples2)
        if args.framework == 'nnclr':
    ...
    if args.framework == 'simclr':
        if args.backbone in ['AE', 'CNN_AE']:
            x1_encoded, x2_encoded, z1, z2 = model(x1=stacked_aug_samples1, x2=stacked_aug_samples2)
            recon_loss = recon(stacked_aug_samples1, x1_encoded) + recon(stacked_aug_samples2, x2_encoded)
        else:
            z1, z2 = model(x1=stacked_aug_samples1.unsqueeze(1), x2=stacked_aug_samples2.unsqueeze(1))
        loss = criterion(z1, z2)
        if args.backbone in ['AE', 'CNN_AE']:
            loss = loss * args.lambda1 + recon_loss * args.lambda2
    if args.framework == 'tstcc':
        nce1, nce2, p1, p2 = model(x1=stacked_aug_samples1, x2=stacked_aug_samples2)
        tmp_loss = nce1 + nce2
...
def test(test_loader, best_model, logger, fitlog, DEVICE, criterion, n_classes, args):
    model, _ = setup_model_optm(args, n_classes, DEVICE, classifier=False)
...
def train_lincls(train_loaders, val_loader, trained_backbone, classifier, logger, fitlog, DEVICE, optimizer, criterion, args):
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, train_loader in enumerate(train_loaders):
            for idx, (sample, target) in enumerate(train_loader):
                print(f"Batch #{idx + 1}")
                sample, target = convert_batch_to_embeddings(sample, DEVICE), target.to(DEVICE).long()
        ...
        print("Validating Linear Classifier")
        with torch.no_grad():
            classifier.eval()
            total_loss = 0
            total = 0
...
def test_lincls(test_loader, trained_backbone, best_lincls, logger, fitlog, DEVICE, criterion, n_classes, args, plt=False):
    classifier = setup_linclf(n_classes, DEVICE, trained_backbone.out_dim)
