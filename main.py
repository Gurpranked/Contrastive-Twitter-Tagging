...
# parser.add_argument('--n_epoch_word2vec', type=int, default=20, help = 'number of training epochs for word2vec auto_encoder') # UNUSED
parser.add_argument('--lr', type=float, default=1e-2, help='learning rate')
parser.add_argument('--lr_cls', type=float, default=1e-2, help='learning rate for linear classifier')
...
# dataset
# parser.add_argument('--dataset', type=str, default='ucihar', choices=['ucihar', 'shar', 'hhar', 'tweets_text'], help='name of dataset')
parser.add_argument('--n_feature', type=int, default=100, help='number of feature dimension')
...
# augmentation
parser.add_argument('--aug1', type=str, default='txt_cnxt_word_aug',
                    choices=['txt_rm_stopwords', 'txt_lwr', 'txt_convert_num', 'txt_expand_contractions', 'txt_cnxt_word_aug'],
                    help='the type of augmentation transformation')
parser.add_argument('--aug2', type=str, default='txt_rm_stopwords',
                    choices=['txt_rm_stopwords', 'txt_lwr', 'txt_convert_num', 'txt_expand_contractions', 'txt_sub_synonym'],
                    help='the type of augmentation transformation')
...
""" NLPAUG models
parser.add_argument('--fasttext_model', type=str, default = 'crawl_300d-2M', choices = ['crawl-300d-2M', 'crawl_300d-2M-subword', 'wiki-news-300d_1M-subword', 'wiki-news-300d-1M'], help = 'model type choice for the fasttext model for NLPAUG augmentation functions')
parser.add_argument('--glove_model' type=str, default = 'glove.6B', help = 'model type choice for the glove model for NLPAUG augmentation functions', choices=['glove.6B', 'glove.42B.300d', 'glove.840B.300d', 'glove.twitter.27B'])
"""
...
if __name__ == '__main__':
    ...
    print('device:', DEVICE, 'dataset: Tweets Text Dataset')

    train_loaders, val_loader, test_loader, n_classes = setup_dataloaders(args)

    model, optimizers, schedulers, criterion, logger, fitlog, classifier, criterion_cls, optimizer_cls = setup(args, n_classes, DEVICE)
    best_pretrain_model = train(train_loaders, val_loader, model, logger, fitlog, DEVICE, optimizers, schedulers, criterion, args)

    best_pretrain_model = test(test_loader, best_pretrain_model, logger, fitlog, DEVICE, criterion, n_classes, args)
